...
void setupGUI()
{
  ...
  fnprompt.setFont(myFont);
  verifyFormat.setFont(myFont);
  quickFormat.setFont(myFont);
  pre_erase.setFont(myFont);
  indexAlign2.setFont(myFont);
  verifyCheck.setFont(myFont);
  ...
  hdRet.setFont(myFont);
  sdRet.setFont(myFont);
...
void drawHistgfx(PGraphics tF, boolean HDImage)
{
  int offset = 3*12;
  tF.beginDraw();
  tF.background(230);
...
void drawFluxwindow(PGraphics tF, int track, boolean HDImage)
{
  if (!showflux) return;
...
void drawHistwindow(PGraphics tF, int track)
{
  if (!showhist) return;
  if (track<0 | track>167) return;
  int xoff = 50;
